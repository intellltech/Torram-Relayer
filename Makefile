...
RANDOM_ADDR = $(shell babylond keys add random_key --keyring-backend test --output json | jq -r '.address')
...
babylond-main-init:
	babylond init torram --chain-id	torram-mainnet
	
babylond-main-start:
	babylond start

babylond-blskey-create:
	@echo "Generated Address: $(RANDOM_ADDR)"
	babylond create-bls-key $(RANDOM_ADDR)
